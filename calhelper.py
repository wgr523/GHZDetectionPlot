...
class CalHelper():
    ...
    def draw_plots(self,output=None,settings='default'):
        N = self.N
        ...
        x = np.arange(1,N+1)
        #if settings!='cc':
        plt.plot(x,to_plot_allC,'o-',label='variable C')
        if settings=='only':
            for i in range(N):
                to_ann=index_C[i]
                to_x=x[i]
                if i==0:
                    to_x-=0.01
                    to_ann=r'$C^*='+str(to_ann)+r'$'
                plt.annotate(to_ann,(to_x,to_plot_allC[i]-0.02))
        if settings=='default':
            plt.plot(x,to_plot_1Cmax,'o-',label='C=|S|')
            plt.plot(x,to_plot_1Cmin,'o-',label='C=|S|,Worst')
        if settings=='cc':
            #print(index_C)
            plt.plot(x,to_plot_1Cmax,'o-',label='C=|S|')
            plt.plot(x,to_plot_2C,'o-',label='C=2|S|')
